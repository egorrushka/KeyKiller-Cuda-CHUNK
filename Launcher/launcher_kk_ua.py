...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("⬡  KeyKiller Launcher -UA")
    ...
    def _build_slider(self,parent):
        ...
        lbl(top,"ДІАПАЗОН ПОШУКУ",font=FUB,fg=ACCENT,bg=PANEL2).pack(side="left")
        self._lock_btn=btn(top,"🔓 ЗАМОК",self._toggle_lock,bg=BORDER,fg=TEXT,abg=PANEL2,
        ...
        btn(prow,"▶ Застосувати",self._apply_pct,bg=ACCENT,fg=BG,abg=ACCENT2,
        ...
        btn(br2,"🗑  Очистити пройдені",self._clear_history,
        ...
        lbl(lg," пройдено  ",font=("Segoe UI",8),fg=MUTED,bg=PANEL2).pack(side="left")
        tk.Frame(lg,bg=ACCENT,width=14,height=7).pack(side="left",pady=2)
        lbl(lg," поточний чанк  🔒=разом",font=("Segoe UI",8),fg=MUTED,bg=PANEL2).pack(side="left")
    ...
    def _build_settings(self,parent):
        ...
        sec_title(inn,"НАЛАШТУВАННЯ",ACCENT,PANEL)

        # exe
        lbl(inn,"ШЛЯХ ДО kk.exe",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w")
        ...
        # Puzzle selector
        lbl(inn,"ВИБІР ПАЗЛУ",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(4,2))
        ...
        lbl(inn,"АДРЕСА ПАЗЛУ (авто)",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(6,2))
        ...
        # Manual chunk HEX
        lbl(inn,"ЧАНК  — ручне введення HEX",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(4,2))
        ...
        btn(inn,"▶ Застосувати чанк",self._apply_hex,bg=ACCENT,fg=BG,abg=ACCENT2,
            font=FUB,px=10,py=4).pack(fill="x",pady=(4,0))
        sep(inn,BORDER,6)

        # Mode
        lbl(inn,"РЕЖИМ ПОШУКУ",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(4,4))
        mf=tk.Frame(inn,bg=PANEL); mf.pack(fill="x",pady=(0,6))
        self._mode=tk.StringVar(value="sequential")
        tk.Radiobutton(mf,text="📋  Послідовний",variable=self._mode,value="sequential",
                       font=FUI,fg=TEXT,bg=PANEL,selectcolor=ENTRY_BG,
                       activebackground=PANEL,cursor="hand2",
                       command=self._on_mode).pack(anchor="w")
        tk.Radiobutton(mf,text="🎲  Випадковий  (-R)",variable=self._mode,value="random",
        ...
        btn(cr2,"💾  Зберегти конфіг",self._save_cfg,bg=BORDER,fg=TEXT,abg=PANEL2,font=FUB,px=10,py=5).pack(side="left")
        btn(cr2,"📂  Завантажити",self._load_cfg_dlg,bg=BORDER,fg=TEXT,abg=PANEL2,font=FUB,px=10,py=5).pack(side="left",padx=(6,0))
    ...
    def _build_test_block(self, parent):
        ...
        tk.Label(tf, text="ТЕСТОВИЙ БЛОК", font=FUB, fg=RED, bg=PANEL2).pack(side="left")
        tk.Label(tf, text="  — ручне введення", font=FUI, fg=MUTED, bg=PANEL2).pack(side="left")

        # Address input
        lbl(inn, "АДРЕСА (Compressed P2PKH)", font=FUH, fg=MUTED, bg=PANEL2).pack(anchor="w")
        ...
        # Mode radiobuttons
        lbl(inn, "РЕЖИМ ТЕСТУ", font=FUH, fg=MUTED, bg=PANEL2).pack(anchor="w", pady=(4,2))
        ...
        self._tbtn = btn(br, "▶  ПРОТЕСТУВАТИ", self._start_test,
                         bg=RED, fg="#ffffff", abg=RED2,
                         font=("Consolas", 10, "bold"), px=14, py=6)
        self._tbtn.pack(side="left")

        self._tstop_btn = btn(br, "■  СТОП", self._stop_test,
        ...
        btn(cr, "💾  Зберегти тест-конфіг", self._save_test_cfg,
            bg=BORDER, fg=TEXT, abg=PANEL2, font=FUB, px=10, py=4).pack(side="left")
        btn(cr, "📂  Завантажити", self._load_test_cfg_dlg,
    ...
    def _start_test(self):
        ...
        if not addr: messagebox.showerror("Помилка", "Введи адресу для тесту."); return
        if not s or not e: messagebox.showerror("Помилка", "Введи Start та End HEX."); return
        import shutil as _sh
        if not os.path.exists(exe) and not _sh.which(exe):
            messagebox.showerror("Файл не знайдено", f"kk.exe не найден:\n{exe}"); return
        ...
        self._t_status.config(text="● Працює...", fg=GREEN)

        self._log("─"*60, "sep")
        self._log("ТЕСТ: " + cmd, "cmd")
        self._log("─"*60, "sep")
    ...
    def _set_test_stopped(self):
        ...
        self._tstop_btn.config(state="disabled")
        self._t_status.config(text="●  Зупинено", fg=MUTED)
    ...
    def _test_watcher(self):
        ...
        try:
            ...
            self.after(0, self._log, f"Тест завершився (код {rc})", "warn")
    ...
    def _handle_test_found(self, content):
        ...
        self._t_status.config(text="🔑  ЗНАЙДЕНО!", fg=GREEN)
        self._log("✅  ТЕСТ: КЛЮЧ ЗНАЙДЕНО! Файл: " + dst, "found")
    ...
    def _build_right(self,parent):
        ...
        self._sbtn=btn(ctrl,"▶  ЗАПУСТИТИ",self._start,bg=GREEN,fg=BG,abg=GREEN2,
                       font=("Consolas",12,"bold"),px=22,py=8)
        self._sbtn.pack(side="left")
        self._xbtn=btn(ctrl,"■  СТОП",self._stop,bg=RED,fg="#fff",abg=RED2,
                       font=("Consolas",12,"bold"),px=18,py=8)
        self._xbtn.pack(side="left",padx=(8,0)); self._xbtn.config(state="disabled")
        self._stlbl=lbl(ctrl,"●  Зупинено",font=("Consolas",10,"bold"),fg=MUTED,bg=BG)
        self._stlbl.pack(side="left",padx=14)
        btn(ctrl,"📂 папка",self._open_folder,bg=BORDER,fg=TEXT,abg=PANEL2,font=FUB,px=10,py=6).pack(side="right")

        # Stats panel
        sp=panel(parent,bd=BORDER); sp.pack(fill="x",pady=(0,6))
        si=tk.Frame(sp,bg=PANEL,padx=12,pady=8); si.pack(fill="x")
        sec_title(si,"ПАРАМЕТРИ  /  ІНФО",ACCENT,PANEL)
        ls=tk.Frame(si,bg=PANEL); ls.pack(fill="x")
        self._i_puz=self._irow(ls,"Пазл:")
        self._i_adr=self._irow(ls,"Адреса:")
        self._i_chk=self._irow(ls,"Чанк:")
        self._i_mod=self._irow(ls,"Режим:")
        self._i_cmd=self._irow(ls,"Команда:")

        # Log
        pb=panel(parent,bd=BORDER); pb.pack(fill="both",expand=True)
        ph=tk.Frame(pb,bg=PANEL,padx=12,pady=6); ph.pack(fill="x")
        sec_title(ph,"ЖУРНАЛ  (kk.exe — в окремому CMD-вікні)",ACCENT,PANEL)
        btn(ph,"✕ очистити",self._clear_log,bg=BORDER,fg=MUTED,abg=PANEL2,font=FUB,px=10,py=2).pack(side="right")
    ...
    def _apply_puzzle(self,num):
        ...
        if self.slider.get_visited():
            if messagebox.askyesno("Зміна пазлу","Очистити історію пройдених ділянок?"):
    ...
    def _toggle_lock(self):
        ...
        if locked: self._lock_btn.config(text="🔒 ЗАМОК  УВІ",fg=ORANGE,bg=PANEL2)
        else:       self._lock_btn.config(text="🔓 ЗАМОК",fg=TEXT,bg=BORDER)
    ...
    def _refresh(self):
        ...
        self._i_mod.set("Випадковий (-R)" if self._mode.get()=="random" else "Послідовний")
    ...
    def _start(self):
        ...
        if not self._addr.get().strip():
            messagebox.showerror("Немає адреси","Вибери пазл зі списку."); return
        ...
        self._stlbl.config(text="● Запуск...",fg=GREEN)
        clo=getattr(self,"_clo_val",self._rs); chi=getattr(self,"_chi_val",self._re)
        self._last_lof=val2frac(clo,self._rs,self._rt)
        self._last_hif=val2frac(chi,self._rs,self._rt)
        cmd=self._build_cmd()
        self._log("─"*60,"sep")
        self._log(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  Запуск  Puzzle #{self._pnum}","warn")
        self._log("Команда: "+cmd,"cmd"); self._log("─"*60,"sep")
        self._save_progress()
        cwd=os.path.dirname(os.path.abspath(__file__))
        try:
            ...
            self._log(f"❌  Помилка: {ex}","found"); self._set_stopped(); return
    ...
    def _stop(self):
        ...
        self._set_stopped(); self._log("⏹  Зупинено вручну.","warn")

    def _set_stopped(self):
        self._running=False; self._sbtn.config(state="normal"); self._xbtn.config(state="disabled")
        self._stlbl.config(text="●  Зупинено",fg=MUTED)
    ...
    def _watcher(self):
        ...
        try:
            ...
            rc=proc.returncode if proc.returncode is not None else "?"
            self.after(0,self._log,f"kk.exe завершився (код {rc})","warn")
    ...
    def _tick(self):
        if self._running: self._stlbl.config(text="● Працює...",fg=GREEN)
    ...
    def _clear_history(self): self.slider.clear_visited(); self._save_history(); self._log("🗑  Історію очищено.","warn")
    ...
    def _record_visited(self):
        if self._last_lof is not None:
            ...
            self._log(f"✔  {self._last_lof*100:.3f}% – {self._last_hif*100:.3f}%  позначено як пройдене.","info")
    ...
    def _save_cfg(self):
        ...
        try:
            ...
            self._flash(self._cfgl,"✓ Збережено")
    ...
    def _load_cfg_dlg(self):
        path=filedialog.askopenfilename(title="Завантажити конфіг",filetypes=[("JSON","*.json"),("All","*.*")])
        if path:
            try:
                with open(path) as f: self._apply_cfg(json.load(f))
                self._flash(self._cfgl,"✓ Завантажено")
            except Exception as ex: messagebox.showerror("Помилка",str(ex))
