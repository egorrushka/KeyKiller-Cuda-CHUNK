...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("⬡  KeyKiller Launcher -RU")
    ...
    def _build_slider(self,parent):
        ...
        lbl(top,"ДИАПАЗОН ПОИСКА",font=FUB,fg=ACCENT,bg=PANEL2).pack(side="left")
        ...
        btn(prow,"▶ Применить",self._apply_pct,bg=ACCENT,fg=BG,abg=ACCENT2,
        ...
        btn(br2,"🗑  Очистить пройденные",self._clear_history,
        ...
        lbl(lg," текущий чанк  🔒=вместе",font=("Segoe UI",8),fg=MUTED,bg=PANEL2).pack(side="left")
    ...
    def _build_settings(self,parent):
        ...
        sec_title(inn,"НАСТРОЙКИ",ACCENT,PANEL)

        # exe
        lbl(inn,"ПУТЬ К kk.exe",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w")
        ...
        # Puzzle selector
        lbl(inn,"ВЫБОР ПАЗЛА",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(4,2))
        ...
        lbl(inn,"АДРЕС ПАЗЛА (auto)",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(6,2))
        ...
        # Manual chunk HEX
        lbl(inn,"ЧАНК  — ручной ввод HEX",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(4,2))
        ...
        btn(inn,"▶ Применить чанк",self._apply_hex,bg=ACCENT,fg=BG,abg=ACCENT2,
            font=FUB,px=10,py=4).pack(fill="x",pady=(4,0))
        sep(inn,BORDER,6)

        # Mode
        lbl(inn,"РЕЖИМ ПОИСКА",font=FUH,fg=MUTED,bg=PANEL).pack(anchor="w",pady=(4,4))
        mf=tk.Frame(inn,bg=PANEL); mf.pack(fill="x",pady=(0,6))
        self._mode=tk.StringVar(value="sequential")
        tk.Radiobutton(mf,text="📋  Последовательный",variable=self._mode,value="sequential",
                       font=FUI,fg=TEXT,bg=PANEL,selectcolor=ENTRY_BG,
                       activebackground=PANEL,cursor="hand2",
                       command=self._on_mode).pack(anchor="w")
        tk.Radiobutton(mf,text="🎲  Случайный  (-R)",variable=self._mode,value="random",
        ...
        btn(cr2,"💾  Сохранить конфиг",self._save_cfg,bg=BORDER,fg=TEXT,abg=PANEL2,font=FUB,px=10,py=5).pack(side="left")
        btn(cr2,"📂  Загрузить",self._load_cfg_dlg,bg=BORDER,fg=TEXT,abg=PANEL2,font=FUB,px=10,py=5).pack(side="left",padx=(6,0))
    ...
    def _build_test_block(self, parent):
        ...
        tk.Label(tf, text="ТЕСТОВЫЙ БЛОК", font=FUB, fg=RED, bg=PANEL2).pack(side="left")
        tk.Label(tf, text="  — ручной ввод", font=FUI, fg=MUTED, bg=PANEL2).pack(side="left")

        # Address input
        lbl(inn, "АДРЕС (Compressed P2PKH)", font=FUH, fg=MUTED, bg=PANEL2).pack(anchor="w")
        ...
        # Mode radiobuttons
        lbl(inn, "РЕЖИМ ТЕСТА", font=FUH, fg=MUTED, bg=PANEL2).pack(anchor="w", pady=(4,2))
        ...
        self._tbtn = btn(br, "▶  ПРОТЕСТИРОВАТЬ", self._start_test,
        ...
        btn(cr, "💾  Сохранить тест-конфиг", self._save_test_cfg,
            bg=BORDER, fg=TEXT, abg=PANEL2, font=FUB, px=10, py=4).pack(side="left")
        btn(cr, "📂  Загрузить", self._load_test_cfg_dlg,
    ...
    def _start_test(self):
        ...
        if not addr: messagebox.showerror("Ошибка", "Введи адрес для теста."); return
        if not s or not e: messagebox.showerror("Ошибка", "Введи Start и End HEX."); return
        import shutil as _sh
        if not os.path.exists(exe) and not _sh.which(exe):
            messagebox.showerror("Файл не найден", f"kk.exe не найден:\n{exe}"); return
        ...
        self._t_status.config(text="● Работает...", fg=GREEN)
    ...
    def _set_test_stopped(self):
        ...
        self._tstop_btn.config(state="disabled")
        self._t_status.config(text="●  Остановлен", fg=MUTED)
    ...
    def _test_watcher(self):
        ...
        try:
            ...
            self.after(0, self._log, f"Тест завершился (код {rc})", "warn")
    ...
    def _handle_test_found(self, content):
        ...
        self._t_status.config(text="🔑  НАЙДЕН!", fg=GREEN)
        self._log("✅  ТЕСТ: КЛЮЧ НАЙДЕН! Файл: " + dst, "found")
    ...
    def _save_test_cfg(self):
        try:
            ...
            self._flash(self._t_cfgl, "✓ Сохранено")
        except Exception as ex:
            messagebox.showerror("Ошибка", str(ex))

    def _load_test_cfg_dlg(self):
        path = filedialog.askopenfilename(
            title="Загрузить тест-конфиг",
            filetypes=[("JSON", "*.json"), ("All", "*.*")])
        if path:
            try:
                with open(path) as f:
                    self._apply_test_cfg(json.load(f))
                self._flash(self._t_cfgl, "✓ Загружено")
            except Exception as ex:
                messagebox.showerror("Ошибка", str(ex))
    ...
    def _build_right(self,parent):
        ...
        self._sbtn=btn(ctrl,"▶  ЗАПУСТИТЬ",self._start,bg=GREEN,fg=BG,abg=GREEN2,
        ...
        self._stlbl=lbl(ctrl,"●  Остановлен",font=("Consolas",10,"bold"),fg=MUTED,bg=BG)
        ...
        sec_title(si,"ПАРАМЕТРЫ  /  LIVE СТАТИСТИКА",ACCENT,PANEL)
        ls=tk.Frame(si,bg=PANEL); ls.pack(fill="x")
        self._i_puz=self._irow(ls,"Пазл:")
        self._i_adr=self._irow(ls,"Адрес:")
        ...
        sec_title(ph,"ЖУРНАЛ  (kk.exe — в отдельном CMD-окне)",ACCENT,PANEL)
        btn(ph,"✕ очистить",self._clear_log,bg=BORDER,fg=MUTED,abg=PANEL2,font=FUB,px=10,py=2).pack(side="right")
    ...
    def _apply_puzzle(self,num):
        ...
        if self.slider.get_visited():
            if messagebox.askyesno("Смена пазла","Очистить историю пройденных участков?"):
    ...
    def _toggle_lock(self):
        ...
        if locked: self._lock_btn.config(text="🔒 ЗАМОК  ВКЛ",fg=ORANGE,bg=PANEL2)
    ...
    def _refresh(self):
        ...
        self._i_mod.set("Случайный (-R)" if self._mode.get()=="random" else "Последовательный")
    ...
    def _start(self):
        ...
        if not self._addr.get().strip():
            messagebox.showerror("Нет адреса","Выбери пазл из списка."); return
        ...
        try:
            ...
            self._log(f"❌  Ошибка: {ex}","found"); self._set_stopped(); return
    ...
    def _stop(self):
        ...
        self._set_stopped(); self._log("⏹  Остановлено вручную.","warn")

    def _set_stopped(self):
        self._running=False; self._sbtn.config(state="normal"); self._xbtn.config(state="disabled")
        self._stlbl.config(text="●  Остановлен",fg=MUTED)
    ...
    def _watcher(self):
        ...
        try:
            ...
            rc=proc.returncode if proc.returncode is not None else "?"
            self.after(0,self._log,f"kk.exe завершился (код {rc})","warn")
    ...
    def _tick(self):
        if self._running: self._stlbl.config(text="● Работает...",fg=GREEN)
    ...
    def _clear_history(self): self.slider.clear_visited(); self._save_history(); self._log("🗑  История очищена.","warn")
    ...
    def _record_visited(self):
        if self._last_lof is not None:
            ...
            self._log(f"✔  {self._last_lof*100:.3f}% – {self._last_hif*100:.3f}%  отмечено как пройденное.","info")
    ...
    def _save_cfg(self):
        ...
        try:
            ...
            self._flash(self._cfgl,"✓ Сохранено")
    ...
    def _load_cfg_dlg(self):
        path=filedialog.askopenfilename(title="Загрузить конфиг",filetypes=[("JSON","*.json"),("All","*.*")])
        if path:
            try:
                with open(path) as f: self._apply_cfg(json.load(f))
                self._flash(self._cfgl,"✓ Загружено")
            except Exception as ex: messagebox.showerror("Ошибка",str(ex))
