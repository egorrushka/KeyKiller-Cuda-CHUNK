...
__global__ void comp_keys(address_t* sAddress, uint32_t* lookup32, uint64_t* keys, uint32_t* out) {
    ...
    // --- 檢查第一個點 ---
    odd_py = sy[0] & 1;
    if ((d_searchInfo & 1) == SEARCH_MODE_PUBKEY) { // 檢查搜索模式
...
GPUEngine::GPUEngine(int gpuId, uint32_t maxFound) {
    ...
    if (!randomMode) {
        ...
        while (powerOfTwo <= nbThreadGroup) {  //  // 获取最接近的 2 的幂
            powerOfTwo <<= 1;
        }
...
// 8891689_MOD: https://github.com/8891689 ，将 SetTargetPublicKey 的实现放在这里 
void GPUEngine::SetTargetPublicKey(const uint64_t* pubKeyX, int pubKeyParity) {
    cudaError_t err;

    // 将搜索模式和奇偶性打包到一個 uint32_t 中
    // bit 0: 1 表示公鑰模式, 0 表示地址模式
    // bit 1: 1 表示奇數 y, 0 表示偶數 y
...
// 8891689_MOD: 确保 SetSearchType 也设置 searchInfo，以便地址模式正常工作
void GPUEngine::SetSearchType(int searchType) {
    this->searchType = searchType;
    
    // 对于地址模式，我们也需要初始化 d_searchInfo
...
void GPUEngine::FreeGPUEngine() {  //釋放 GPU 以用於 Pause 函數
